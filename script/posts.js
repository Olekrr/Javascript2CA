...
function populateEditModal({ title, body, id }) {
    ...
    const updateButton = document.querySelector('#updatePostForm button[type="submit"]');
...
function filterPostsBySearch(keyword) {
    const filteredPosts = localPosts.filter(post => post.title.toLowerCase().includes(keyword.toLowerCase()));
